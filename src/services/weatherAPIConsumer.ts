...
class WeatherAPIConsumer {
  ...
  public constructor() {
    const weatherData = WEATHER_DATA as {stations: Station[]; parameters: Parameter[]; measurements: Measurement[]};
    this._stations = weatherData.stations;
    this._parameters = weatherData.parameters;
    this._measurements = {};
    for (const station of this.getStations()) {
      this._measurements[station.id] = weatherData.measurements
