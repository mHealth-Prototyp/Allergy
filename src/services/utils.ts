 ...
 */
function filterByDate(diaryEntries: DiaryEntry[], range: TimestampRange): DiaryEntry[] {
  diaryEntries = diaryEntries.filter((x) =>
    date.isBetweenDates(x.date, range.from, range.to, {
      onlyDate: true,
      inclusiveFrom: true,
      inclusiveTo: true
    })
...
/**
 * Parses ISO 8601 dates' strings to Date. Use as reviver parameter for JSON.parse method.
 * @param key The key of the JSON value.
 * @param value The JSON value of the key.
 * @returns A Date if the conditions were met or the value itself.
 */
function jsonParseReviver(key: string, value: unknown) {
...
/**
 * Replaces undefined values by null to allow them to be converted in JSON instead of beeing ignored.
 * @param key The key of the JSON value.
 * @param value The JSON value of the key.
 * @returns A null if the condition was met or the value itself.
 */
